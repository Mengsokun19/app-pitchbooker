...
exports.findAll = async (req, res) => {
    ...
    Pitch_price.findAll({
        ...
        .then((pitch_prices) => {
            ...
            // console.log(pitch_prices.length);
        })
        .catch((err) => {
            res.status(500).send({
                message:
                    err.message ||
                    'Some error occurred while retrieving pitch price.',
            });
        });
};
...
exports.calculatePrice = async (req, res) => {
    ...
    try {
        ...
        // console.log(week_parts);

        // Get all day parts
        const dayParts = await sequelize.query(
            'SELECT id, name, "sportClubId" FROM day_parts WHERE start_time < (:time_out) and end_time > (:time_in)',
            {
        ...
        var price = await sequelize.query(
            'SELECT (SUM(price)/(:length)) * (:hours) * 2 AS total_prices FROM pitch_prices WHERE "dayPartId" IN(:dayPartIds) and "weekPartId" = (:weekPartId)',
            {
                replacements: {
                    hours: total_hours,
        ...
        res.status(500).send({
            message: 'Error calculating pitch price!',
        });
    }
};
